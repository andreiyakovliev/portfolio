 ...
 */
    ...
    function menuInit() {
        ...
        document.addEventListener("click", (function(e) {
            ...
            if (menuIsOpen && !e.target.closest(".menu") && !e.target.closest(".icon-menu")) menuClose(); else if (e.target.closest(".icon-menu")) if (menuIsOpen) menuClose(); else menuOpen();
        }));
    }
    function menuOpen() {
        bodyLockToggle();
        document.documentElement.classList.add("menu-open");
        document.querySelector(".icon-menu").classList.add("active-menu");
    }
    function menuClose() {
        bodyLockToggle();
        document.documentElement.classList.remove("menu-open");
        document.querySelector(".icon-menu").classList.remove("active-menu");
    }
    function showMore() {
    ...
    function initSliders() {
        if (window.innerWidth < 767.98 && document.querySelector(".about__slider")) {
    ...
    function handleBlockMovement() {
        ...
        elements.forEach((element => {
            ...
            if (targetElement && !isNaN(width) && !isNaN(position)) if (window.innerWidth <= width) if (position === 0) targetElement.prepend(element); else if (position === -1) targetElement.append(element); else {
                const children = Array.from(targetElement.children);
                const referenceElement = children[position] || null;
                if (referenceElement) targetElement.insertBefore(element, referenceElement); else targetElement.append(element);
            }
        }));
    }
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        if (widget) if (getComputedStyle(widget).display === "none") {
            const blogSection = document.querySelector(".page__blog.blog");
            if (blogSection) blogSection.style.display = "none";
        }
    }));
    function copyTextToClipboard(text) {
        if (navigator.clipboard) return navigator.clipboard.writeText(text); else {
    ...
    document.getElementById("contactForm").addEventListener("submit", (function(event) {
        event.preventDefault();
        emailjs.sendForm("service_z4dkx3g", "template_p34z5xo", this).then((function(response) {
    ...
    const texts = {
        en: {
            ...
            projects: {
                ...
                "card-5": {
                    ...
                    content: `   <p>Responsive website with using VSCode and Figma</p>\n                                 <p>Tab navigation to view new information on each page</p>\n                                 <p>Responsive design for optimal display on various devices</p>\n                                 <p>Animated hover effects for interactive elements</p>\n                                 <p class="card-projects__text text"><span>Tech stack:</span> HTML,\n                                    CSS, JavaScript</p>`
                }
            },
            blog: {
                title: "Blog"
        ...
        uk: {
            ...
            footer: {
                ...
                mail: {
                    title: "Мій email",
                    content: `< p > andrii.yakovliev.frontend</>\n            <p>@gmail.com</p>`
                },
                phone: {
            ...
            signature: `Дизайн та розробка < span > Андрій Яковлєв</ > ❤️ 2024`
        },
        ru: {
